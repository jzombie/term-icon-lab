...
# glob would pair page_10's image with page_2's sidecar rows). Exit 1 =
# legitimate per-glyph verification failures — the verdicts are the
# deliverable, so only usage/internal errors (>= 2) fail the run.
PNG_ARGS=()
i=0
